...
export function CalendarPanel({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const el = document.querySelector(
        `[data-event-id="${highlightedEventId}"], .fc-event[data-id="${highlightedEventId}"]`
      ) as HTMLElement | null;

      if (el) {
        gsap.fromTo(
          el,
          { boxShadow: "0 0 0px oklch(0.65 0.2 260 / 0)" },
          {
            boxShadow: "0 0 18px oklch(0.65 0.2 260 / 0.9), 0 0 36px oklch(0.65 0.2 260 / 0.4)",
            duration: 0.6,
            repeat: 5,
            yoyo: true,
            ease: "power2.inOut",
  ...
  useEffect(() => {
    if (!isLoading) {
      gsap.fromTo(".calendar-shell", { opacity: 0.4 }, { opacity: 1, duration: 0.5, ease: "power2.out" });
    }
  }, [isLoading]);
    ...
    <div className="glass rounded-2xl overflow-hidden min-w-0">
      <div className="flex flex-col gap-3 p-6 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">从后端读取当前可见范围内的日程。</p>
        </div>
      ...
      <div className="px-6 pb-6 space-y-3">
        ...
        <div className="calendar-shell relative min-h-[620px] rounded-md p-3">
          {isLoading ? (
